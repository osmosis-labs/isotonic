...
github_changelog_generator -u osmosis-labs -p isotonic --base CHANGELOG.md $ORIGINAL_OPTS || cp /tmp/CHANGELOG.md.$$ CHANGELOG.md

if [ -n "$UPCOMING_TAG" ]
then
  # Add "upcoming" version tag
  TODAY=$(date "+%Y-%m-%d")
  sed -i "s+\[Full Changelog\](https://github.com/osmosis-labs/isotonic/compare/\(.*\)\.\.\.HEAD)+[Full Changelog](https://github.com/osmosis-labs/isotonic/compare/$UPCOMING_TAG...HEAD)\n\n## [$UPCOMING_TAG](https://github.com/osmosis-labs/isotonic/tree/$UPCOMING_TAG) ($TODAY)\n\n[Full Changelog](https://github.com/osmosis-labs/isotonic/compare/\1...$UPCOMING_TAG)+" CHANGELOG.md
fi
